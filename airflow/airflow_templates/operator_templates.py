...
class OperatorTemplate:
    ...
    def create_dbt_docker_operator(main_dbt_command, task_id, additional_env_vars=None):
        ...
        all_env_vars_passed_to_container = env_vars | (
        ...
        full_dbt_command = "{main_cmd} {suffix} ".format(
            suffix=suffix, main_cmd=main_dbt_command
        )
        ...
        return DockerOperator(
            ...
            mount_tmp_dir=False,
            mounts=[
                Mount(source="dbt-logs-volume", target="/opt/dbt/logs", type="volume"),
                Mount(
                    source="dbt-target-volume", target="/opt/dbt/target", type="volume"
                ),
            ],
    ...
    @staticmethod
    def create_python_script_docker_operator(
        task_id, command, additional_env_vars=None
    ):
        env_vars = {
